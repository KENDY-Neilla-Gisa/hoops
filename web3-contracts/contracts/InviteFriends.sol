...
contract InviteFriends {
    ...
    constructor(address _hoopsToken) {
        hoopsToken = IERC20(_hoopsToken);
    }

    function invite(address invitee, uint256 amount) external {
        require(!invited[invitee], "Already invited");
        require(hoopsToken.balanceOf(msg.sender) >= amount, "Insufficient token balance");
    ...
    function acceptInvitation() external {
        ...
        hoopsToken.transfer(msg.sender, amount);
